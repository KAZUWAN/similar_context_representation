...
class BatchManager():
    ...
    def get_sameslot_sentence(self, anchor_slot):
        ...
        if type(anchor_slot) is list:
            select_sameslot_index = np.random.randint(0, len(anchor_slot), size=1)[0]
            sameslot_sentence_list = self.sentence_list_by_keys[anchor_slot[select_sameslot_index]]
        else:
    ...
    def get_difference_slot_sentence_list(self, anchor_slot):
        ...
        if len(difference_slottypes) == 0:
            ...
            select_different_slot_index = np.random.randint(0, len(difference_slottypes), size=1)[0]
            negative_candidate_list = self.sentence_list_by_keys[difference_slottypes[select_different_slot_index]]
