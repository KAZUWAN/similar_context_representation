...
class KNNManager():
    ...
    def run_knn(self, title, k_range_start=1, k_range_end=100, k_range_step=1):
        ...
        plt.savefig(f"./log/{title}_knn_{k_range_start}_{k_range_end}_{k_range_step}_result.png")
        plt.show()
        ...
        best_k_range = k_range[index]
        print("「k="+str(best_k_range)+"」の時、適合率は最大値「"+str(max_precision)+"」をとる")

        max_recall = max(self.recall_list)
        index        = self.recall_list.index(max_recall)
        best_k_range = k_range[index]
        print("「k="+str(best_k_range)+"」の時、再現率は最大値「"+str(max_recall)+"」をとる")
            ...
            for i in range(len(train_sentences["sentence_list"])):
                ...
                for j in range(len(train_slot_list[i])):
                    if train_slot_list[i][j] != 'o': # 文脈後以外を訓練データに取り込む
                        ...
                        self.y_train.append(train_slot_list[i][j])
                    prog.progress_bar(int(i//(30)), int(len(train_sentences["sentence_list"]))//(30))
            print()
            ...
            for i in range(len(test_sentences["sentence_list"])):
                ...
                for j in range(len(test_slot_list[i])):
                    if test_slot_list[i][j] != 'o':
                        ...
                        self.y_test.append(test_slot_list[i][j])
                    prog.progress_bar(int(i//(30)), int(len(test_sentences["sentence_list"]))//(30))
            print()
