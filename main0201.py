...
from src.knn_slot_estimator0101 import KNNManager
...
def eval(generate_semantic_setntence_embedding_model):
    ...
    model_knn.data_arrangement(model=trained_model, train_sentences=train_sentences, test_sentences=test_sentences, train_slot_list =creater_train.ori_slot_list, test_slot_list=creater_test.ori_slot_list)
    model_knn.run_knn("trained model", k_range_start=1, k_range_end=20, k_range_step=1)
    print("\n\n########    EVALUATION ORIGINAL MODEL    ########\n\n")
    model_knn = KNNManager()
    model_knn.data_arrangement(model=original_model, train_sentences=train_sentences, test_sentences=test_sentences, train_slot_list =creater_train.ori_slot_list, test_slot_list=creater_test.ori_slot_list)
